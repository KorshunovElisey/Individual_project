...
writeValue = np.array([], dtype=np.int64)
...
while True:
    ...
    cv2.circle(img, (100, 100), 50, RED, 2)
    ...
    for cnt in contours0:
        if len(cnt) > 30:
            ...
            angle = 180.0 / math.pi * math.acos(
                (referenceNew[0] * usedEdge[0] + referenceNew[1] * usedEdge[1]) / (cv2.norm(referenceNewTouple) * cv2.norm(usedEdge))
            )
    ...
    cv2.imshow('f', img)
    if counter:
        cv2.createTrackbar('shift', 'd', 0, 180, on_change)
        cv2.createTrackbar('factor', 'd', 0, 1, on_change)
        cv2.createTrackbar('area_min', 'd', 0, 50000, on_change)
        cv2.createTrackbar('area_max', 'd', 50000, 50000, on_change)
    ...
    factor = cv2.getTrackbarPos('factor', 'd')
...
for i in range(0, len(timeList), 5):
    ...
    writeValue = np.append(writeValue, int(filtered[i]))
